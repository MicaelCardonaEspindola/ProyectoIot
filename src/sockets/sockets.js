import { obtenerUsuarioPorId } from "../components/user/user.models.js";
...
export class Sockets {
  ...
  socketEvents() {
    this.io.on('connection', async (socket) =>  { 
        // Validar JWT y obtener el ID del cliente
        const [valido, verified] = validateJsonWebToken(socket.handshake.query['x-token']);
        if (!valido) {
            return socket.disconnect();
        }

        // console.log(`Usuario conectado: ${verified.email}`);
        socket.join(verified.id.toString());

        // Manejar eventos de 'join-room'
        socket.on('join-room', async (room) => {
            socket.join(room);
            // console.log(`Usuario ${verified.nombre_completo} se unió a la sala ${room}`);
        });
        ...
        socket.on('leave-room', async (room) => {
            socket.leave(room);
            // console.log(`Usuario ${verified.nombre_completo} dejó la sala ${room}`);
        });
        ...
        socket.on('enviar-invitacion', async (data) => {
            ...
            const existeUsuario = await obtenerUsuarioPorId(para);
            
            if (!existeUsuario) {
                return socket.emit('error', 'El usuario no existe, revise el ID');
            }
        ...
        socket.on('disconnect', () => {
            // console.log(`Usuario ${verified.email} desconectado`);
            socket.disconnect();
        });
    });
  }
